...
@Repository
public interface StockRepositorySensor extends JpaRepository<SensorStock,Long> {
...
//            "           order by date desc")
    ...
    @Query ("SELECT stock from SensorStock stock where datetime between  :fromDate AND :toDate")
    Optional<List<SensorStock>> findAllRecordSensorByDate(LocalDateTime fromDate, LocalDateTime toDate);
